...
df["City"]        = df["City"].apply(lambda x: str(x).strip() if (x == x and str(x).strip().lower() not in ('nan','none','')) else None)
...
# ── 5. Meta Ads insights ──────────────────────────────────────────────────────
meta_api_key    = env.get("meta_api_key")    or os.environ.get("META_API_KEY")
meta_account_id = env.get("meta_account_id") or os.environ.get("META_ACCOUNT_ID")
meta_data       = {"byDay": {}, "totalSpend": 0, "totalImpressions": 0, "totalLpv": 0, "totalClicks": 0}

if meta_api_key and meta_account_id:
    print("Fetching Meta Ads insights…")
    since = CAMPAIGN_START.strftime("%Y-%m-%d")
    today = datetime.date.today().strftime("%Y-%m-%d")
    r = requests.get(
        f"https://graph.facebook.com/v21.0/{meta_account_id}/insights",
        params={
    ...
    for row in r.json().get("data", []):
        ...
        lpv         = next((int(a["value"]) for a in row.get("actions", [])
                            if a["action_type"] == "landing_page_view"), 0)
        by_day[date_str] = {"spend": spend, "impressions": impressions, "lpv": lpv, "clicks": clicks}
    meta_data = {
        "byDay":            dict(sorted(by_day.items())),
        "totalSpend":       round(sum(v["spend"] for v in by_day.values()), 2),
        "totalImpressions": sum(v["impressions"] for v in by_day.values()),
        "totalLpv":         sum(v["lpv"] for v in by_day.values()),
        "totalClicks":      sum(v["clicks"] for v in by_day.values()),
    }
    print(f"  → {len(by_day)} days, €{meta_data['totalSpend']:.2f} spend, "
          f"{meta_data['totalImpressions']:,} impressions, {meta_data['totalLpv']:,} LPVs")
else:
    print("⚠️  No Meta API key or account ID — skipping Meta data")

# ── 6. Write data.js ──────────────────────────────────────────────────────────
output = {"records": records, "stripe": stripe_data, "meta": meta_data}
